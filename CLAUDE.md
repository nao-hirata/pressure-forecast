...
- **一覧と3日ミニグラフ** — `renderLogs()` が `#logsPanel`（`#content` の外なので気圧取得失敗時も生きる）にカードを描画。カードのタップはパネルへのイベント委譲で処理（展開時は先に表示→`drawMiniChart()` の順で、コンテナ幅を確定させてから描く）。カードを開くと `drawMiniChart()` が記録日を含む3カレンダー日（`miniWindow()`、記録日が右端）の SVG を描き、記録時刻に縦マーカーを打つ。**メイングラフと同一構成・同一縮尺**（`H=320`・横密度 4.6px/h・共通 `yRange`・メインと同じ `pressure-line`/`gridline`/`axis-label`/`day-label` クラス）で描くので低下傾斜を直接見比べられる。メイン同様の**固定Y軸 + 横スクロール本体の2レイヤー**（`.log-mini-yaxis` が不透明背景 `var(--panel-2)` で本体左端を隠し、`.log-mini-scroll` が横スクロール、初期位置は記録日付近へ寄せる）で、Y軸ラベルはスクロールに追従しない。**気圧急降下区間はメインと同じ `detectDrops()`** を窓内に適用し同色の赤ハイライト矩形を敷く（`fromIdx=0`＝窓内の低下を全て対象）。**長押し/なぞりでの詳細表示もメインと同じ**（本体 SVG の `.mini-hit` 透明矩形＋`.mini-dot`＋`.mini-tip`、ローカル要素で完結させ複数カード同時展開でも衝突しない。`move()`・350ms 長押し・読み取り中のみ `preventDefault()` の挙動は `drawChart()` と同一）。データは保存済み `series` を優先し、無ければ `ALL` から切り出す（どちらも不可なら「取得範囲外」）。メモの編集はカードを開いた下部の「✏️ メモを編集」から `<dialog id="editDialog">` を開き、`store.update(id, { memo })` で本文だけ更新する（日時・`pressure`・`series` のスナップショットは変えない＝過去の気圧履歴を壊さない）。削除はその隣の「🗑 記録を削除」から `<dialog id="delDialog">` で確認してから `store.remove()` を実行する。
